...
app.config['MAIL_USERNAME'] = 'ebfd11ce068e05'
app.config['MAIL_PASSWORD'] = '199080fc5f964f'
...
def db_seed():
    ...
    test_user=User(first_name="Shubham Kumar", last_name="Gupta", password="Shubham", email='shubhamkumargupta2@gmail.com')
...
def super_simple():
    return jsonify(message='Hello from planetory API.')
...
def parameters():
    ...
    if age<18:
        return jsonify(message="Sorry "+name+". You can't access this"),401
    else:
        return jsonify(message="Welcome "+ name+" to our site"),200
...
def retrieve_password(email:str):
    ...
    if user:
        msg=Message("Your password is: "+user.password,sender='admin@panetary-api.com',recipients=[email])
        mail.send(msg)
        return jsonify(message="Password sent to"+ email)
    else:
       return jsonify(message="Mail does not exist") ,401 


@app.route('/planet_details/<string:planet>',methods=['GET'])
def planet_details(planet:str):
    planet=Planet.query.filter_by(planet_name=planet).first()
    if planet:
        result=planet_schema.dump(planet)
        return jsonify(result)
    else:
        return jsonify(message="Please check the spelling"),404

@app.route('/add_planet',methods=['POST'])
@jwt_required
def add_planet():
    planet_name=request.form['planet_name']
    test=Planet.query.filter_by(planet_name).first()
    if test:
...
def update_planet():
    ...
    if planet:
        ...
        planet.home_star=request.form['planet_home_star']
